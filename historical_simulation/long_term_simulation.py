...
def run_long_term_simulation():
    print("\n" + "="*80)
    print("🚀 STARTING 480-DAY REAL NEWS TRADING SIMULATION")
    print("="*80)

    # 1. Configuration
    STOCKS = ["RELIANCE.NS", "TCS.NS", "INFY.NS", "HDFCBANK.NS", "ICICIBANK.NS"]
    INITIAL_BALANCE = 100000 # Increased for long term
    SEQUENCE_LENGTH = 30
    INPUT_DIM = 9 
    ...
    try:
        ...
        if os.path.exists(active_model_txt):
            ...
            # Fallback to any model in the directory
            models = [f.replace(".zip", "") for f in os.listdir(model_dir) if f.endswith(".zip") and "multi_stock" in f]
            if models:
                active_model_path = os.path.join(model_dir, models[-1])
            else:
    ...
    for ticker in STOCKS:
        ...
        # apply feature engineering (normalization + indicators)
        # the prepare_simulation_data already has some columns, but we need indicators
    ...
    for step in range(SEQUENCE_LENGTH, env.max_steps):
        ...
        # De-normalize sentiment for display if it was normalized (0-1 -> -1 to 1)
        # Actually in our prepare script we keep it raw, but get_feature_df might normalize it.
    ...
    plt.plot(pd.to_datetime(df_history['date']), df_history['portfolio_value'], label='Portfolio Value', color='#2ecc71', linewidth=2)
    plt.axhline(y=INITIAL_BALANCE, color='red', linestyle='--', label='Initial Balance')
    plt.title(f'480-Day AI Trading Simulation (Real News Sentiment)', fontsize=14)
    plt.xlabel('Date')
    plt.ylabel('Portfolio Value (INR)')
    plt.grid(True, alpha=0.3)
    plt.legend()
    
    report_path = 'historical_simulation/simulation_results.png'
